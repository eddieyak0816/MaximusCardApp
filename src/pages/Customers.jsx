...
function Customers() {
  ...
  // Check if we were sent here to open a specific customer
  useEffect(() => {
    if (!loading && location.state && location.state.customerId && customers.length > 0) {
        const targetId = location.state.customerId;
        const targetCustomer = customers.find(c => c.id === targetId);
        
        if (targetCustomer) {
            handleEditClick(targetCustomer);
  ...
  useEffect(() => {
    if (showScanner) {
        const scanner = new Html5QrcodeScanner("reader-popup", { fps: 10, qrbox: 250 }, false);
        scanner.render(
            (decodedText) => {
                scanner.clear();
                setNewCardId(decodedText.trim());
  ...
  const fetchLinkedCards = async (customerId) => {
    try {
        ...
        setLinkedCards(querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
  ...
  const processedData = useMemo(() => {
    let data = [...customers];
    if (startDate) { const start = new Date(startDate + 'T00:00:00'); data = data.filter(c => c.createdDate >= start); }
    if (endDate) { const end = new Date(endDate + 'T23:59:59'); data = data.filter(c => c.createdDate <= end); }
    if (searchTerm) {
  ...
  const handleDelete = async (id, name) => {
    if (window.confirm(`⚠️ Delete customer: ${name}?\n\nNote: Any cards linked to this customer will stay in the system but will become "Unlinked".`)) {
      ...
      } catch (error) { alert("Error deleting customer."); }
    }
  };
  ...
  const handleEditClick = (customer) => {
    ...
    setEditForm({
      ...
      address: customer.address || '', city: customer.city || '', state: customer.state || '', zip: customer.zip || '', notes: customer.notes || '', newsletter: customer.newsletter || false
    });
    setNewCardId(""); setNewCardPin("");
    fetchLinkedCards(customer.id);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const handleUpdate = async () => {
    if (!editForm.firstName || !editForm.lastName) { alert("First and Last Name are required."); return; }
    try {
      await updateDoc(doc(db, "customers", editingId), editForm);
      alert("Customer updated!");
      setEditingId(null); setLinkedCards([]); fetchCustomers(); 
    } catch (error) { console.error(error); alert("Error updating customer."); }
  };

  const handleCancelEdit = () => { setEditingId(null); setLinkedCards([]); };

  const handleAddCard = async () => {
    if (!newCardId || newCardPin.length !== 4) { alert("Please enter a Card ID and a 4-digit PIN."); return; }
    try {
        const cardRef = doc(db, "cards", newCardId);
        const cardSnap = await getDoc(cardRef);
        if (cardSnap.exists()) { alert("⚠️ Error: This Card ID is already registered in the system!"); return; }
        await setDoc(cardRef, { cardId: newCardId, pin: newCardPin, balance: 0.00, customerId: editingId, created: Timestamp.now() });
        alert("✅ Card Added Successfully!");
        setNewCardId(""); setNewCardPin(""); fetchLinkedCards(editingId);
    } catch (error) { console.error(error); alert("Error creating card."); }
  };
  ...
  return (
    <div style={{ padding: '20px', width: '100%', maxWidth: '1000px', margin: '0 auto', textAlign:'center' }}>
      ...
      {showScanner && (
        <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.9)', zIndex:999, display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center'}}>
            <h2 style={{color:'white'}}>Scan Card QR Code</h2>
            <div id="reader-popup" style={{width:'300px', height:'300px', background:'white'}}></div>
            <button 
                onClick={() => setShowScanner(false)} 
                style={{marginTop:'20px', padding:'10px 20px', background:'red', color:'white', border:'none', fontSize:'18px', cursor:'pointer'}}
            >
                Cancel / Close
            </button>
        </div>
      ...
      {editingId && (
        <div style={{textAlign:'left', background:'#f0f0f0', color:'black', padding:'20px', borderRadius:'8px', marginBottom:'20px', border:'2px solid #007bff'}}>
          ...
          <div style={{marginTop:'10px', display:'flex', gap:'10px'}}>
             <label style={{display:'flex', alignItems:'center', gap:'5px', cursor:'pointer'}}>
                <input type="checkbox" checked={editForm.newsletter} onChange={e => setEditForm({...editForm, newsletter: e.target.checked})} /> Opt-in Newsletter
             </label>
          </div>

          <div style={{marginTop:'20px', borderTop:'1px solid #ccc', paddingTop:'10px'}}>
             <h4 style={{margin:'0 0 10px 0'}}>💳 Linked Gift Cards</h4>
             {linkedCards.length === 0 ? <p style={{fontStyle:'italic', color:'#666', fontSize:'13px'}}>No cards linked yet.</p> : (
                    ...
                    <tbody>
                        {linkedCards.map(c => (
                            <tr key={c.id} style={{borderBottom:'1px solid #eee'}}>
                                <td style={{padding:'8px', fontWeight:'bold', textAlign:'left'}}>{c.id}</td>
                                <td style={{padding:'8px', color: c.balance > 0 ? 'green' : 'black', textAlign:'left'}}>${c.balance.toFixed(2)}</td>
                                <td style={{padding:'8px', textAlign:'center'}}>
                                    <button onClick={() => navigate(`/card-logic/${c.id}`)} style={{background:'#28a745', color:'white', border:'none', padding:'4px 8px', borderRadius:'3px', cursor:'pointer', fontSize:'12px'}}>VIEW</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
             ...
             <div style={{background:'#e9ecef', padding:'10px', borderRadius:'5px', display:'flex', gap:'10px', alignItems:'center', flexWrap:'wrap'}}>
                ...
                <button onClick={() => setShowScanner(true)} style={{background:'#007bff', color:'white', border:'none', padding:'8px 12px', borderRadius:'4px', cursor:'pointer', fontSize:'16px'}} title="Open Camera">📷</button>
                <input placeholder="Scan/Type Card ID" value={newCardId} onChange={e => setNewCardId(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddCard()} style={{...inputStyle, width:'140px', textTransform:'uppercase'}} />
                <input type="number" placeholder="Set PIN" value={newCardPin} onChange={e => setNewCardPin(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddCard()} style={{...inputStyle, width:'80px'}} />
                <button onClick={handleAddCard} style={{background:'#28a745', color:'white', border:'none', padding:'8px 16px', borderRadius:'4px', cursor:'pointer', fontWeight:'bold'}}>ADD</button>
             </div>
          </div>
                ...
                processedData.map((customer) => (
                  <tr key={customer.id} style={{ borderBottom: '1px solid #ddd', background: editingId === customer.id ? '#eef7ff' : 'white' }}>
                    ...
                    <td style={{padding:'10px', textAlign:'center', display:'flex', gap:'5px', justifyContent:'center'}}>
                      <button onClick={() => handleEditClick(customer)} title="Edit Customer" style={{background:'transparent', border:'1px solid #ccc', borderRadius:'4px', fontSize:'20px', cursor:'pointer', padding:'5px'}}>✏️</button>
                      <button onClick={() => handleDelete(customer.id, customer.firstName)} title="Delete Customer" style={{background:'transparent', border:'1px solid #ffcccc', borderRadius:'4px', fontSize:'20px', cursor:'pointer', padding:'5px'}}>❌</button>
                    </td>
                  </tr>
                ))
            )}
